...
# Это объявление уже было отправлено до переноса на VDS. Сохраняем прежний
# идентификатор, чтобы первый серверный запуск не публиковал его повторно.
RELEASE_ID = "2026-07-31-museum-help-layout"
RELEASE_SUMMARY = """🆕 Обновление бота

🏛 Добавлен музей

• Ва-банк приносит 1 золото за каждые полные 100 000 очков ставки.
• При выпадении ровно двух 7️⃣ золото не начисляется.
• Золото отображается в команде «каз баланс».
• Команда «музей» показывает свою экспозицию, а ответом на сообщение —
  музей другого игрока.
• Статуи создаются командами «музей создать 10 золота Б»,
  «музей создать 10з Б» или «музей создать Б 10 з».
• Команда «музей помощь» подробно объясняет формулы и качества статуй.
• Доход всей экспозиции начисляется за каждый завершённый час."""
