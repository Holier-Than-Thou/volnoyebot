...
# Новый идентификатор позволяет один раз показать сводку в каждом активном
# разделе после установки этой версии.
RELEASE_ID = "2026-08-05-motovskikh-tests"
RELEASE_SUMMARY = """🆕 Обновление бота

🗺 Добавлены дуэли на тестах Motovskikh

• Привяжите аккаунт Motovskikh: в личном чате с ботом отправьте
  /motovskikh_auth и подтвердите найденный аккаунт кнопкой.
• Вызов соперника: «мот <ставка> <ссылка на тест>» ответом на его
  сообщение либо с его @тегом. Ставку можно не указывать.
• Соперник принимает вызов ответом «+» или «да» в течение 60 секунд.
• Бот создаёт приватную комнату, посторонних в неё не пускает и после
  завершения теста сам начисляет банк победителю.
• «мот инфо» — ваша статистика, «мот топ» — рейтинг чата."""
